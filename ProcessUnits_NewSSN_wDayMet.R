...
required_packages <- c('here','SSN2','dplyr','tidyr','doParallel')
...
for(u in c(1,2,3,5,6,7,8)){
  ...
  DayMet_Obs_AnnualPrecip <- DayMet_Obs %>% 
    select(ID_1KM,year,yday,prcp..mm.day.) %>%
    mutate(WaterYear = ifelse(yday>= 274,year+1,year)) %>%
    group_by(WaterYear,ID_1KM) %>% 
    summarise(AnnualPrecip = sum(prcp..mm.day.)) %>% 
    rename(SAMPLEYEAR = WaterYear)
  ...
  OriginalPrecipData <- unit_df %>% select(ID_1KM,SAMPLEYEAR,PRECIP)
...
#   DayMet_Preds<-lapply(1:length(PredFiles), FUN=function(x){
#     #x<-list.files(paste('/Volumes/Cline_USGS/DayMet',UnitName,'PredsBy1000',sep='/'))[1]
#     t1<-readRDS(paste('/Volumes/Cline_USGS/DayMet',UnitName,'PredsBy1000',PredFiles[x],sep='/'))
#     t2 <- t1 %>% filter(month=='08') %>% 
#       select(ID_1KM,year,yday,month,tmax..deg.c.,tmin..deg.c.) %>%
#       mutate(tmean..deg.c = tmin..deg.c. + (tmax..deg.c.-tmin..deg.c.)/2) %>%
#       group_by(year,ID_1KM) %>% 
#       summarise(AugMeanTemp = mean(tmean..deg.c)) %>% 
...
#       rename(SAMPLEYEAR = WaterYear)
#     
#     return(t2 %>% left_join(p2,by=c('SAMPLEYEAR','ID_1KM')))
#     
#   }) %>% bind_rows()
#   
#   
#   lapply(PredPtNames,FUN=function(x){
#     #x<-PredPtNames[1]
#     
#     unit_df_preds <- getSSNdata.frame(UnitIn_DayMet,x)
#     
#     OriginalAirData <- unit_df_preds %>% select(ID_1KM,SAMPLEYEAR,Air_Aug)
#     CombiAirTemp <- OriginalAirData %>% left_join(DayMet_Preds %>% select(ID_1KM,SAMPLEYEAR,AugMeanTemp),by=c('ID_1KM','SAMPLEYEAR'))
#     unit_df_preds$Air_Aug <- CombiAirTemp$AugMeanTemp
#     
#     OriginalPrecipData <- unit_df_preds %>% select(ID_1KM,SAMPLEYEAR,PRECIP)
#     CombiPrecip <- OriginalPrecipData %>% left_join(DayMet_Preds %>% select(ID_1KM,SAMPLEYEAR,AnnualPrecip),by=c('ID_1KM','SAMPLEYEAR'))
#     unit_df_preds$PRECIP <- CombiPrecip$AnnualPrecip
#     
#     UnitIn_s <- putSSNdata.frame(unit_df_preds,UnitIn_s,x)
#     
#   })
#   
#   
